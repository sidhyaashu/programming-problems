/*
isBipartiteUtil() function is used to perform BFS traversal and color the vertices such that no adjacent vertices have the same color.
isBipartite() function checks if the graph is bipartite by calling isBipartiteUtil() for each connected component.
The main function creates a graph, adds edges, prints the graph structure, and then checks if the graph is bipartite.
*/
